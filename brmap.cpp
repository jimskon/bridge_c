...
void brmap::print() {
    ...
    for(; it != bridge.end(); it++)
      {
	_log(0) << it->first;
	_log(0) << " I: " << it->second.src_interface << " VID: " << it->second.vid << endl;
      }
  }
  ...
  /* Transparent learning bridge map
     ...
     -1 - not found, drop
  */
int brmap::map_pkt(int pkt_src, unsigned char *packet) {
...
void brmap::add_vid(unsigned char * mac, uint32_t v) {
...
void *socket_listener(void * arg) {
	...
	error("ERROR on accept bridge assignments");
      ...
      if (n < 0) error("ERROR reading from bridge socket");
...
void brmap::start_listener(int port) {
  pthread_t fifo_thrd;
